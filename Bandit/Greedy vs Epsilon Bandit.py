...
class GreedyAgent(main_agent.Agent):
    def agent_step(self, reward, observation):
        ...
        if reward > 0:
            consequence = self.q_values[self.last_action] + (step_size)*(reward - self.q_values[self.last_action])
        else:
            consequence = 0
        self.q_values[self.last_action] = consequence
...
class EpsilonGreedyAgentConstantStepsize(main_agent.Agent):
    def agent_step(self, reward, observation):
        ...
        self.arm_count[self.last_action] += 1
        if reward > 0:
            consequence = self.q_values[self.last_action] + (self.step_size)*(reward - self.q_values[self.last_action])
        else:
            consequence = 0
        self.q_values[self.last_action] = consequence
